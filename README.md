...
這些已經在 `.gitignore` 或前端 `.gitignore` 中處理。真正需要上傳的是：
...
目前版本適合課程專題與 demo，不建議直接當正式校務系統公開使用。

主要限制：

- 登入/註冊目前是前端展示流程，沒有正式後端認證。
- 管理員權限目前沒有後端 middleware 保護。
- CORS 目前允許所有來源。
- Docker Compose 目前是本機 demo 設定，資料庫帳密仍是 demo 用。
- 系統只檢核目前實作的畢業規則，不代表學校正式畢業審查結果。
...
前端只負責操作與呈現。
後端負責資料處理、規則計算與儲存。
資料庫只由後端透過 Sequelize 存取。
```

這樣切分可以讓專案比較好維護，也方便之後補正式登入、權限管理、正式部署或更多學年度規則。
